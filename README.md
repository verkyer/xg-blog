...
- [x] 🏷️ 文章分类 + 标签，内容井井有条
- [x] 🔍 站内全文搜索（基于 [`Pagefind`](https://github.com/pagefind/pagefind)，按需加载）
- [x] 🧭 Banner、菜单等配置自由定制
- [x] 🔗 友情链接轻松管理
...
- [x] 🌗 深色 / 浅色主题一键切换
- [x] 🎨 主题色可自定义，色彩随心
- [x] 🗨️ 评论系统（基于 [`Twikoo`](https://github.com/twikoojs/twikoo)，文章、页面可按需开启）
- [ ] 📃 可视化编辑器（计划中）
...
`comments` 字段默认按开启处理；只有写 `comments: false` 时才关闭评论。评论系统是否真正显示，还需要在 `.env` 或部署平台环境变量里配置 `TWIKOO_ENV_ID`。
...
项目使用 [`Twikoo`](https://github.com/twikoojs/twikoo) 作为评论系统，详细教程可看[官方文档](https://twikoo.js.org/backend.html)。（推荐：[Vercel](https://twikoo.js.org/backend.html#vercel-%E9%83%A8%E7%BD%B2)、[Netlify](https://twikoo.js.org/backend.html#netlify-%E9%83%A8%E7%BD%B2)、[Docker](https://twikoo.js.org/backend.html#%E7%A7%81%E6%9C%89%E9%83%A8%E7%BD%B2-docker) 部署）

- `TWIKOO_ENV_ID` 是评论系统的服务地址；
- 每篇文章或页面的 `comments` 字段决定这一页要不要显示评论区。

```text
显示评论 = TWIKOO_ENV_ID 已填写 + 当前文章/页面没有写 comments: false
```

启用步骤：

1. 复制 `.env.example` 为 `.env`。
2. 在 `.env` 里填写你的 Twikoo 地址，例如：

```env
TWIKOO_ENV_ID="https://twikoo.example.com"
```

3. 想关闭某篇文章或某个页面的评论时，在它的 Markdown 属性区写：
...
程序默认固定使用 Twikoo `1.7.12`。`TWIKOO_JS_URL` 一般不用改；没有配置或留空时，会使用默认 jsDelivr 地址。

|使用场景|`TWIKOO_JS_URL`|
|-|-|
|默认|`https://cdn.jsdelivr.net/npm/twikoo@1.7.12/dist/twikoo.min.js`|
|国内可选 1|`https://registry.npmmirror.com/twikoo/1.7.12/files/dist/twikoo.min.js`|
|国内可选 2|`https://s4.zstatic.net/npm/twikoo@1.7.12/dist/twikoo.min.js`|

> 文章浏览量计数，也是基于 `Twikoo` 实现。
...
|`TWIKOO_JS_URL`|`https://cdn.jsdelivr.net/npm/twikoo@1.7.12/dist/twikoo.min.js`|Twikoo 前端脚本 CDN 地址，默认`1.7.12` 版本；国内访问不稳定时可换成上方可选地址|
