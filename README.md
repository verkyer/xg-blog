...
本项目使用 Twikoo。简单理解：`TWIKOO_ENV_ID` 是评论系统的服务地址；每篇文章或页面的 `comments` 字段决定这一页要不要显示评论区。
...
```env
TWIKOO_ENV_ID="https://twikoo.xiaoge.org"
```
...
默认会从 `TWIKOO_JS_URL` 加载 Twikoo 前端脚本；一般不用改。如果你的访问环境打不开默认 CDN，可以把它换成其他 `twikoo.min.js` 地址。
...
|`TWIKOO_JS_URL`|`https://cdn.jsdelivr.net/npm/twikoo@1.7.12/dist/twikoo.min.js`|Twikoo 前端脚本地址；默认值通常不用改|
